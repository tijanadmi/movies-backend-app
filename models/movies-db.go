...
func (m *DBModel) Get(id int) (*Movie, error) {
	...
	query := `select id, title, description, year, release_date, rating, runtime, mpaa_rating,
				created_at, updated_at, coalesce(poster, '') from postgres.public.movies where id = $1
	`
...
func (m *DBModel) DeleteMovie(id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	log.Println("Movie is deleted", id)
...
// Get returns 0 in user is invalid or 1 if user is in database
func (m *DBModel) CheckUser(user User) ( error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	
	query := `select count(*) from postgres.public.users where username = $1 and password = $2`
	
	row := m.DB.QueryRowContext(ctx, query, user.Email, user.Password)
